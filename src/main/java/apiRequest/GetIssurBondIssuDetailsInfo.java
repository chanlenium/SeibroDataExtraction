...
public class GetIssurBondIssuDetailsInfo {
	public ArrayList<BondCompanyModel> getIssurBondIssuDetailsInfo(GeneralCompanyModel company) throws IOException{
		...
		StringBuilder urlBuilder = new StringBuilder("http://api.seibro.or.kr/openapi/service/BondSvc/" + className); /*URL*/
        try {
			urlBuilder.append("?" + URLEncoder.encode("ServiceKey","UTF-8") + "=" + Servicekey.getServicekey());
			//urlBuilder.append("&" + URLEncoder.encode("issucoCustno","UTF-8") + "=" + URLEncoder.encode("7", "UTF-8")); /*페이지번호*/
